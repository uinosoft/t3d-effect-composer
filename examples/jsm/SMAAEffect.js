import { ShaderPostPass, Texture2D, TEXTURE_FILTER } from 't3d';
import { Effect } from 't3d-effect-composer';

/**
 * Subpixel Morphological Antialiasing (SMAA) Effect.
 * Preset: SMAA 1x Medium (with color edge detection)
 * References:
 * - {@link https://github.com/iryoku/smaa/releases/tag/v2.8}
 * - {@link https://github.com/mrdoob/three.js/blob/dev/examples/jsm/shaders/SMAAShader.js}
 */
export class SMAAEffect extends Effect {
	...
	constructor() {
		...
		areaTextureImage.onload = function() {
			scope._areaTexture.version++;
		};

		this._areaTexture = new Texture2D();
		this._areaTexture.image = areaTextureImage;
		this._areaTexture.minFilter = TEXTURE_FILTER.LINEAR;
		...
		searchTextureImage.onload = function() {
			scope._searchTexture.version++;
		};

		this._searchTexture = new Texture2D();
		this._searchTexture.image = searchTextureImage;
		this._searchTexture.magFilter = TEXTURE_FILTER.NEAREST;
		this._searchTexture.minFilter = TEXTURE_FILTER.NEAREST;
		this._searchTexture.generateMipmaps = false;
		this._searchTexture.flipY = false;

		// passes

		this._edgesPass = new ShaderPostPass(smaaEdgesShader);

		this._weightsPass = new ShaderPostPass(smaaWeightsShader);
		this._weightsPass.uniforms.tArea = this._areaTexture;
		this._weightsPass.uniforms.tSearch = this._searchTexture;

		this._blendPass = new ShaderPostPass(smaaBlendShader);
	...
	dispose() {
		this._edgesPass.dispose();
		this._weightsPass.dispose();
